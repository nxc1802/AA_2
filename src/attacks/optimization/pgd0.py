...
class PGD0Attack:
    """Authentic PGD-0 Attack with exact hard spatial $L_0$-ball projection."""
    ...
    def attack(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for step in range(self.steps):
            ...
            # Unprojected update on perturbation delta
            candidate_delta = delta + self.alpha * grad.sign()
            
            # Projection onto exact L0 spatial ball of size K
            delta = project_l0(candidate_delta, self.k)
            x_adv_proj = torch.clamp(x + delta, 0.0, 1.0)

            with torch.no_grad():
                out_step = self.model(x_adv_proj)
                ...
                best_adv[improved] = x_adv_proj[improved]
