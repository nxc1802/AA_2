...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data in enumerate(data_loader):
            ...
            # perturb = self.attack.perturb(x, y, l0=False)
            # x_adv = x + perturb
...
class TrainerSAT(Trainer):
    def __init__(self, model, attack, optimizer, summary_writer=None,
                 print_freq=1, output_freq=1, is_cuda=True, base_lr=0.1,
                 max_epoch=100, steps=[], rate=1., loss='adv', trades_beta=1., scheduler=None, mode='rand', sat_epoch=50):
        super(TrainerSAT, self).__init__(model, attack, optimizer, summary_writer, print_freq, output_freq, is_cuda, base_lr,
              max_epoch, steps, rate, loss, trades_beta, scheduler, mode)
        self.soft_label = None
        self.sat_epoch = sat_epoch

    def ready_data(self, dataset, num_class):
        print("Ready Data...")
        self.num_class = num_class
        self.soft_label = torch.zeros(len(dataset), num_class)
        # for i, (x, y, idx) in tqdm(enumerate(dataset), leave=False, total=len(dataset)):
        #     self.soft_label[idx, y] = 1

    def train(self, epoch, data_loader):
        # assert self.soft_label is not None, "please run ready_data before training"
        ...
        for i, data in enumerate(data_loader):
            ...
            x, y, index = data
            if epoch > 0:
                y_soft = self.soft_label[index]
            else:
                y_soft = torch.eye(self.num_class)[y]
            ...
            x_adv, acc, iter = self.attack.perturb(x, y_soft)
            # perturb = self.attack.perturb(x, y, l0=False)
            # x_adv = x + perturb
            adv_time.update(time.time() - t0)

            if self.loss == 'adv':
                adv_loss, adv_pred, y_soft = self.adv_loss(x_adv, y_soft, epoch)
            else:
                adv_loss, adv_pred, y_soft = self.trades_loss(x, x_adv, y_soft, epoch)

            self.soft_label[index] = y_soft.cpu().clone()
    ...
    def adv_loss(self, x, y, epoch):
        adv_pred = self.model(x)
        if epoch < self.sat_epoch:
            adv_loss = F.cross_entropy(adv_pred, y)
        else:
            y = 0.9*y + 0.1*F.softmax(adv_pred.detach(), dim=1)
            w = torch.max(y, dim=1)[0]
            adv_loss = w * F.cross_entropy(adv_pred, y, reduction='none')
            adv_loss = adv_loss.sum() / torch.sum(w)
        return adv_loss, adv_pred, y

    def trades_loss(self, x, x_adv, y, epoch):
        clean_pred = self.model(x)
        if epoch < self.sat_epoch:
            clean_loss = F.cross_entropy(clean_pred, y)
        else:
            y = 0.9*y + 0.1*F.softmax(clean_pred.detach(), dim=1)
            w = torch.max(y, dim=1)[0]
            clean_loss = w * F.cross_entropy(clean_pred, y, reduction='none')
            clean_loss = clean_loss.sum() / torch.sum(w)
        adv_pred = self.model(x_adv)
        robust_loss = F.kl_div(F.log_softmax(adv_pred, dim=1), F.softmax(clean_pred, dim=1), reduction='batchmean')
        loss = (clean_loss + self.trades_beta*robust_loss) / (1+self.trades_beta)
        return loss, adv_pred, y